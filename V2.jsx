...
async function cp(t) { try { await navigator.clipboard.writeText(t); return true; } catch { return false; } }

// ── ZIP encoder (no deps — STORE method, valid .zip) ──────────────────────────
const CRC_TABLE = (() => {
...
function makeZip(fileMap) {
  ...
  const entries = Object.entries(fileMap).map(([path, content]) => ({
    ...
    dataBytes: enc.encode(content),
  }));
  entries.forEach(e => { e.crc = crc32(e.dataBytes); });
  ...
  for (const f of entries) {
    ...
    dv.setUint32(18, f.dataBytes.length, true);
    dv.setUint32(22, f.dataBytes.length, true);
    dv.setUint16(26, f.nameBytes.length, true);
    h.set(f.nameBytes, 30);
    chunks.push(h, f.dataBytes);
    ...
    central.push(cd);
    offset += h.length + f.dataBytes.length;
  }
...
function extractJSON(text) {
  ...
  try { return JSON.parse(t.slice(0, end)); } catch { return null; }
}

function extractCodeBlock(text) {
  const m = text.match(/```[\w.\-/]*\s*\n([\s\S]*?)```/);
  if (m) return m[1].trim();
  // No fence — strip any leading prose line, return rest
  return text.trim();
}
